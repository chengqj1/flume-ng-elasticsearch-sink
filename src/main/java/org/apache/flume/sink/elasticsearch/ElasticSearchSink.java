...
public class ElasticSearchSink extends AbstractSink implements Configurable {
  ...
  public Status process() throws EventDeliveryException {
    ...
    //System.out.println("flume begin");
    ...
    try {
      ...
      for (num = 0; num < batchSize; ++num) {
        ...
        //System.out.println("event body:"+num);
        //System.out.println(event.);
        //System.out.println(event.getHeaders().entrySet().toString());
        if(jsonField.length()>0){
        	String body =new String(event.getBody());
        	String  fields [] = jsonField.split(",");
            String strs[]=body.split(jsonSplitFlag);
            //StringBuffer  buffer=new StringBuffer();
            
            //buffer.append("{ ");
            if(fields.length<=strs.length){
            	//ok++;
                XContentBuilder builder =XContentFactory.jsonBuilder()  
                    	.startObject() ; 
            	for(int i=0;i<fields.length;i++){
            		String tmp=strs[i].replaceAll("\"", "¡°").replaceAll(":", "£º").replaceAll("\\{", "").replaceAll("\\}", "");
            		//System.out.println(tmp);
            		
            		builder.field(fields[i], tmp)  ;
            		/*
            		if(i==0)
            			buffer.append("\""+fields[i] + "\":\""+ strs[i].replaceAll("\"", "¡°")+"\"");
            		else
            			buffer.append(",\""+fields[i] + "\":\""+strs[i].replaceAll("\"", "¡°")+"\"");
            			*/
            	}
            	SimpleDateFormat format=new SimpleDateFormat("yyyy-MM-dd");           	
            	builder.field("sys_time", format.format(new Date()))  ;
            	builder.endObject();    
                 //System.out.println("buffer51:"+builder.string());
                event.setBody(builder.string().getBytes());
            }
            else{
            	System.out.println("event body321:"+new String(event.getBody()));
            	continue;
            }
      ...
      //System.out.println("all:"+all+" ok:"+ok+" **************count"+count +" batch"+ batchSize);
      txn.commit();
  ...
  public void configure(Context context) {
    ...
    if (StringUtils.isNotBlank(context.getString("jsonField"))) {
        this.jsonField = context.getString("jsonField");
      }
    
    if (StringUtils.isNotBlank(context.getString("jsonSplitFlag"))) {
        this.jsonSplitFlag = context.getString("jsonSplitFlag");
      }
